...
<script>
    export default {
        ...
        mounted() {
          ...
          this.$axios.get('/resapi/martyr/'+this.$route.query.name).then(res=>{
            let data = res.data
              let dict = {};
              dict['name'] = data['name'];
              dict['url'] = data['pictureurl'];
              dict['title'] = data['eventsdate'];
              dict['content'] = data['maincontent'];
              var re=/\r\n/;
              dict['content']=dict['content'].replace(re,"\n");
...
<style scoped>
  ...
  .main-bg{
    min-width: 1400px;
    height: 100%;
    background-image: url("https://redsocialpractice.oss-cn-shanghai.aliyuncs.com/front_images/model_people_bg.jpg");
